...
class Agent:
    ...
    def __init__(self, min_scale: int = 0, max_scale: int = 1, resolution: int = 15,
        ...
        self.x_scaler = MinMaxScaler(feature_range=(self.min_scale, self.max_scale))
        self.y_scaler = MinMaxScaler(feature_range=(self.min_scale, self.max_scale))
    ...
    def transform_df(self, df: pd.DataFrame) -> pd.DataFrame:
        x, y = df.drop(self.target, axis=1), df[[self.target]]
        trans_x = self.x_scaler.transform(x)
        trans_y = self.y_scaler.transform(y)
        if self.verbose:
            print(f"DF before transformation:\n{df.head(20)}")
        result_df = pd.DataFrame(trans_x, columns=x.columns)
        # Add y to the df
        result_df[self.target] = trans_y
        if self.verbose:
            print(f"DF after transformation:\n{result_df.head(20)}")
        return result_df

    def inverse_transform_y(self, data: pd.DataFrame) -> np.ndarray:
    ...
    def predict(self, df: pd.DataFrame, index_to_predict: int, model: Sequential = None) -> float:
        ...
        if self.verbose:
            print(f"Predicting index {index_to_predict} based on previous DF segment:\n{df.loc[row_start_index:index_to_predict]}\n")
    ...
    def predict_n_timesteps(self, df: pd.DataFrame, model=None, n_timesteps: int=None) -> np.ndarray:
        ...
        for i in range(self.start_index, self.start_index+n_timesteps):
            ...
            if self.verbose:
                print(f"Prediction for index {i}:\n{result}")
    ...
    def visualize_results(self, y_true: pd.DataFrame, y_pred: np.ndarray, n_timesteps: int=None) -> None:
        if n_timesteps is None:
            n_timesteps = self.pred_timesteps
        y_true = self.inverse_transform_y(y_true)[self.start_index:self.start_index+n_timesteps]
...
if __name__ == '__main__':
    ...
    print(f"DF Val head:\n{df_val.head(agent.n_prev)}")
    agent.train(df_train, epochs=1)
    agent.predict_n_timesteps(df_val, n_timesteps=4)